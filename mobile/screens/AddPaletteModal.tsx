...
const AddNewPaletteModal = ({ navigation }) => {
  ...
  const [colorToggles, setColorToggles] = useState<{ [name: string]: boolean }>(
    {},
  );

  const handleSubmit = () => {
    if (!name) {
      Alert.alert('Please enter a name for the color profile');
      return;
    }
    const selectedColors = Object.keys(colorToggles)
      .filter((colorName) => colorToggles[colorName] === true)
      .map((colorName) =>
        COLORS.find((color) => color.colorName === colorName),
      );

    if (selectedColors.length < 5) {
      Alert.alert('Please select at least five colors');
      return;
    }
  ...
  return (
    <View style={styles.container}>
      <Text>Name of your color profile</Text>
        ...
        renderItem={({ item }) => {
          return (
            <View style={styles.colorRow}>
              ...
              <View
                style={[styles.swatch, { backgroundColor: item.hexCode }]}
              />
              <Switch
                value={!!colorToggles[item.colorName]}
                onValueChange={(val) => {
                  setColorToggles((cur) => ({
                    ...cur,
                    [item.colorName]: val,
